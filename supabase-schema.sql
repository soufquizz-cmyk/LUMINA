...
-- Ordre manuel des chaînes par bouquet (liste live admin).
create table if not exists public.admin_package_channel_order (
  country_id uuid not null references public.admin_countries (id) on delete cascade,
  package_id text not null,
  stream_order bigint[] not null default '{}',
  updated_at timestamptz not null default now(),
  primary key (country_id, package_id)
);

create index if not exists admin_package_channel_order_package_id_idx
  on public.admin_package_channel_order (package_id);

alter table public.admin_package_channel_order enable row level security;

drop policy if exists "open read/write admin_package_channel_order" on public.admin_package_channel_order;
